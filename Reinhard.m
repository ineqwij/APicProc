function est_im = Reinhard(source,target,target2)
...
img_t2 = reshape(im2double(target2),[],3);
...
mean_t = mean(lab_t,2);
std_t = std(lab_t,0,2);
mean_t2 = mean(lab_t2,2);
std_t2 = std(lab_t2,0,2);
res_lab = zeros(3,x*y);
...
for ch = 1:3 % for each channel, apply the statistical alignment
    res_lab(ch,:) = (lab_s(ch,:) - mean_s(ch))*sft(ch) + mean_t2(ch);
end
